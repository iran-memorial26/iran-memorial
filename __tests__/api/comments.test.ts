...
const { mockFindMany, mockCommentCreate, mockVictimFindUnique, mockRateLimit } =
  vi.hoisted(() => ({
    mockFindMany: vi.fn(),
    mockCommentCreate: vi.fn(),
    mockVictimFindUnique: vi.fn(),
    mockRateLimit: vi.fn(),
  }));
...
vi.mock("@/lib/db", () => ({
  prisma: {
    ...
    victim: {
      findUnique: mockVictimFindUnique,
    },
  },
}));
...
describe("GET /api/comments", () => {
  ...
  it("returns comments with null authorName", async () => {
    const res = await GET(createGetRequest({ victimId: "victim-123" }));
    const json = await res.json();
    expect(json.comments[1].authorName).toBeNull();
    expect(json.comments[1].content).toBe("We will never forget");
  });

  it("queries only approved comments ordered by createdAt desc", async () => {
  ...
  it("passes different victimId values correctly", async () => {
    await GET(createGetRequest({ victimId: "abc-def-ghi" }));
    expect(mockFindMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({ victimId: "abc-def-ghi" }),
      })
    );
  });
...
describe("POST /api/comments", () => {
  ...
  describe("Rate Limiting", () => {
    ...
    it("does not create comment when rate limited", async () => {
      mockRateLimit.mockResolvedValue({
        success: false,
        remaining: 0,
        resetAt: Date.now() + 3600000,
  ...
  describe("Input Validation", () => {
    ...
    it("accepts content at exactly 3 characters", async () => {
      const res = await POST(
        createPostRequest({
          victimId: "victim-123",
          content: "abc",
    ...
    it("returns 400 when content is an empty string", async () => {
      const res = await POST(
        createPostRequest({ victimId: "victim-123", content: "" })
      );
      expect(res.status).toBe(400);
    });

    it("returns 400 when content is whitespace only shorter than 3 chars", async () => {
      const res = await POST(
        createPostRequest({ victimId: "victim-123", content: "  " })
      );
      expect(res.status).toBe(400);
  ...
  describe("Victim Existence Check", () => {
    ...
    it("does not create comment when victim is not found", async () => {
      mockVictimFindUnique.mockResolvedValue(null);
  ...
  describe("Successful Comment Creation", () => {
    ...
    it("stores content at max 2000 characters", async () => {
      await POST(
        createPostRequest({
          victimId: "victim-123",
    ...
    it("accepts Farsi content", async () => {
      await POST(
        createPostRequest({
          victimId: "victim-123",
          content: "روحش شاد",
    ...
    it("accepts comment without authorName field at all", async () => {
      const res = await POST(
        createPostRequest({
          victimId: "victim-123",
